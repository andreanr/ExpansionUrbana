...
GenerateFeature <- function(config, experiment){
  ...
  query = sprintf("CREATE TABLE  features.%s_%s AS (
                  ...
                  FROM hex_grid_250.aeropuertos
                  JOIN hex_grid_250.carreteras
                  USING (cell_id)
                  JOIN hex_grid_250.censos
                  USING (cell_id)
                  JOIN hex_grid_250.centro_urbano
                  USING (cell_id)
                  JOIN hex_grid_250.denue
                  USING (cell_id)
                  JOIN hex_grid_250.localidades_rurales_distance
                  USING (cell_id, year)
                  JOIN hex_grid_250.slope
                  USING (cell_id)
                  JOIN hex_grid_250.vias_ferreas
                  USING (cell_id)
                  JOIN hex_grid_250.zonas_urbanas_distancia
                  USING (cell_id, year)
                  JOIN hex_grid_250.vecinos_urbanos
                  USING (cell_id, year)
                ) ", features_table_name, 
                     grid_size,
                     select_statement)
                  ...
                  WITH intersect_2000 AS (
                      ...
                      SELECT cell_id,
                            '2010'::TEXT AS year,
                              sum(st_area(ST_Intersection(geom, cell)) / st_area(cell)) as porcentage_ageb_share
                      FROM hex_grid_250.grid
                      LEFT JOIN preprocess.urbano_2015
                      ON st_intersects(cell, geom)
                      GROUP BY cell_id
                ), labels_2010 AS (
                      SELECT cell_id,
