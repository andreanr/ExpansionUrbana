...
cat $EXTRACTED_DATA_FOLDER"/ITER_08XLS10.csv" | dos2unix |tr [:upper:] [:lower:] | csvsql -e latin1 -i "postgresql" --no-constraints --tables raw.iter_2010 | tr -d "\"" > $EXTRACTED_DATA_FOLDER"/iter2010.sql"
...
psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "Copy raw.iter_2010 from '$EXTRACTED_DATA_FOLDER/ITER_08XLS10.csv' DELIMITER ',' CSV header;"
#
### 2005
...
cat $EXTRACTED_DATA_FOLDER"/ITER_08XLS05.csv" | dos2unix | csvsql -e latin1 -i "postgresql" --no-constraints --tables raw.iter_2005 | tr -d "\"" > $EXTRACTED_DATA_FOLDER"/iter2005.sql"
...
psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "Copy raw.iter_2005 from '$EXTRACTED_DATA_FOLDER/ITER_08XLS05.csv' DELIMITER ',' CSV header;"
#
# 2000
...
cat $EXTRACTED_DATA_FOLDER"/ITER_08XLS00.csv" | dos2unix | csvsql -e latin1 -i "postgresql" --no-constraints --tables raw.iter_2000 | tr -d "\"" > $EXTRACTED_DATA_FOLDER"/iter2000.sql"
...
psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "Copy raw.iter_2000 from '$EXTRACTED_DATA_FOLDER/ITER_08XLS00.csv' DELIMITER ',' CSV header;"
