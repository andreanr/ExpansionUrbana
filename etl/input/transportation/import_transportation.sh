...
shp2pgsql -s 909090  -d -D -I $EXTRACTED_DATA_FOLDER"/Aeropuerto.shp"  raw.aeropuertos | psql -d $DB_NAME -h $DB_HOST -U  $DB_USER

################### Carreteras #########################
yes | unzip $RAW_DATA_FOLDER/carreviali08.zip -d $EXTRACTED_DATA_FOLDER
shp2pgsql -s 909090 -d -D -I -W "latin1"  $EXTRACTED_DATA_FOLDER"/Red_de_Carreteras_08_Chih.shp" raw.carreteras | psql -d $DB_NAME -h $DB_HOST -U $DB_USER

################## Vías Férreas #######################
yes | unzip $RAW_DATA_FOLDER/viaFerrea.zip -d $EXTRACTED_DATA_FOLDER
shp2pgsql  -s 909090 -d -D -I $EXTRACTED_DATA_FOLDER"/ViaFerrea.shp" raw.via_ferrea | psql -d $DB_NAME -h $DB_HOST -U $DB_USER
