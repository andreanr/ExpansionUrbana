...
ColumnNames <- function(config, table_name){
  ...
  query = sprintf( "SELECT column_name
                   ...
                   WHERE table_schema = 'hex_grid_250'
                   AND table_name = '%s'
...
CollapseCensos <- function(config){
  ...
  query_drop = c("DROP TABLE IF EXISTS hex_grid_250.censos")
  query = sprintf("CREATE TABLE hex_grid_250.censos AS (
                  SELECT cell_id, year, %s
                  FROM hex_grid_250.censos_urbanos as c 
                  JOIN hex_grid_250.censos_rurales as r 
